export const SHIP_LIGHTING_PALETTE_SKILL_ID = 'ship-interior-lighting-palette@1.0.0';
...
export function buildShipLightingPalettePrompt(sourceLabel: string): string {
  return [
    `任务：分析参考图 1“${sourceLabel}”的真实像素颜色，生成一张专业的飞船内景灯光色表。`,
    ...
    '左上角标题必须逐字为：飞船内景灯光色表  /  Extracted Color Palette',
    ...
    '质量硬约束：中文必须完整清晰，无乱码、错别字或方框；HEX 与 RGB 必须彼此一致；不得重复色；A 区必须 20 色，B 区必须 4 色；不要自行改变任何固定标题、栏目名称或网格结构。',
  ].join('\n');
}
